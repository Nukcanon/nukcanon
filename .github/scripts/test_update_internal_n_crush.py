...
def fixture(extra=None, corrupt=False, guide=False):
    ...
    manifest = {'version': '1.2.1', 'threads': False, 'source_commit': COMMIT, 'files': [{'path': p, 'bytes': len(c), 'sha256': hashlib.sha256(c).hexdigest()} for p, c in files.items()]}
    if corrupt:
    ...
    return data, {'version': '1.2.1', 'sha256': hashlib.sha256(data).hexdigest(), 'source_commit': COMMIT}
...
class InstallerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.page = '<a href="play/">Play</a><a href="' + installer.RELEASE + 'InternalNCrush_Windows_v1.2.1.zip">Download</a>'
    ...
    def test_guide_installs_with_verified_screenshots(self):
        ...
        installer.install(self.root, data, expected)
        self.assertIn('v1.2.1 · Windows', (self.root / 'internal-n-crush.html').read_text())
