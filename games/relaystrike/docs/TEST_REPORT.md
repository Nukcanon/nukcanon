...
Internal N Crush 0.3.0 / Godot 4.4.1
...
- AI·조준·모델 검사 38/38 통과: 이동·연사·공중·조준 오차, T 패턴, 6병과 12클립 및 관절 변형, 메딕 회복/2초 간격, 공병 포탑/강화/엄폐물/수리, 새 장애물 경로, 연막/섬광, 각 병과 스킬, 회복·후퇴·보급 판단, 벽 시야 차단, 난이도별 반응.
- 실제 물리 시뮬레이션 봇 시험 6/6 통과: 맵 횡단 장애물 회피, 시작점 이탈, 자율 점령, 설치와 해체.
- 실제 ENet 서버/클라이언트를 별도 Linux 프로세스로 실행. 클라이언트가 장비 변경을 서버에 예약하고, 서버가 탈락 처리한 뒤 부활 상태를 동기화. 클라이언트의 관전 입력 이후 카메라 오차 0.005m 미만, 공병 PULSE와 FIX 적용, 서버에 전송된 사격으로 탄창 6→5 확인.
