...
func run():
	...
	var a=g.actors[1];var p=g.players[1];a.position=Vector3(25,0,63);a.reset_view(0)
	g.add_player(-1,"TARGET","target");g.actors[-1].position=Vector3(25,0,53);g.actors[-1].set_team(1-p.team);g.players[-1].team=1-p.team
