...
func run():
	...
	var manifest={"license":"Original Internal N Crush assets; see LICENSE.txt","operators":[],"weapons":[],"animations":["idle","walk","run","crouch","crouch_walk","jump","fall","fire","reload","hit","land","death"]}
	for role in range(6):
		for team in range(2):
	...
	for id in Catalog.weapons:
		var visual=WeaponVisual.new();root.add_child(visual);visual.build(Catalog.get_weapon(id),false)
	...
	print("MODELS_BUILT operators=12 weapons=27 clips=12_per_operator");quit()
